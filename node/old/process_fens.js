...
//assign those x and y values and and push back into an object so we can get them via a fen string
 	var central_stem = false;
	if(central_stem){
		var xPos = Math.floor(width_max/2);
		var xPos_original = xPos;
	}else{
		xPos = Math.floor((width_max/2 - row_width/2));
	}
	var turn_object = {};
...
var out_file = fs.openSync('../generated_data/anand_chart_positions_left.json', 'w');
